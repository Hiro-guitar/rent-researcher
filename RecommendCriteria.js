...
function listRecommendCriteria(customerName) {
  ...
  // ⚠️ 読む幅は、下で参照する一番右の列（AO=41: 最低階数）まで取ること。
  //   36列しか読んでいなかったため、独立洗面台モード(AK=37)が常に空として返り、
  //   顧客管理ページのプルダウンが「選別（除外）」でも「アラートのみ」に見えていた。
  //   検索側(_appendRecommendCriteria_)は getDataRange() で全列読むので動きは正しく、
  //   画面の表示だけが実態とズレていた（2026-09-20 修正）。
  var data = sheet.getRange(2, 1, sheet.getLastRow() - 1, RECOMMEND_COL_MINFLOOR).getValues();
  var out = [];
