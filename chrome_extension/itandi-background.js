...
async function searchItandiForCustomer(tabId, customer, seenIds, searchId) {
  ...
  const _skipCacheKey = customer.recommend
    ...
    : customer.name;
  const skipStorageKey = `itandiSkipped_${_skipCacheKey}`;
  const skipHashKey = `itandiSkipHash_${_skipCacheKey}`;
      ...
      if (_normKey) {
        ...
        if (earlyKey) {
          ...
          if (_compSkipped && _compSkipped.has(earlyKey)) {
            await setStorageData({ debugLog: `[itandi] ${customer.name}: ✗ 競合スキップ(早期): ${prop.building_name || ''} ${prop.room_number || ''} (詳細遷移省略)` });
            continue;
          }
