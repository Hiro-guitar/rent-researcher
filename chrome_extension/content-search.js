...
(function () {
  ...
  // === 物件番号オートサーチ（Discord等のリンクから #bukken=XXXX で起動） ===
  let __autoSearchedBukken = null;
  const BUKKEN_KEY = 'pendingBukkenSearch';
  function getBukkenFromHash() {
    const m = (location.hash || '').match(/bukken=(\d+)/);
    return m ? m[1] : null;
  }
  // hashで来た物件番号は即座にstorageへ退避（ログイン挟むと消えるため）
  (function captureHash(){
    const n = getBukkenFromHash();
    if (n) {
      try { chrome.storage.local.set({ [BUKKEN_KEY]: n }); } catch(e) {}
      // hash消す（再実行ループ回避）
      try { history.replaceState(null, '', location.pathname + location.search); } catch(e) {}
    }
  })();
  async function autoSearchByBukken() {
    let num = null;
    try {
      const r = await new Promise(res => chrome.storage.local.get([BUKKEN_KEY], res));
      num = r?.[BUKKEN_KEY] || null;
    } catch(e) {}
    if (!num || num === __autoSearchedBukken) return;
    // ログイン画面では遷移しない
    if (location.href.includes('login') || location.href.includes('GKG001')) return;
    // 検索フォームページへ誘導
    if (!location.href.includes('GBK004100')) {
      __autoSearchedBukken = num; // ループ防止
      location.assign('https://system.reins.jp/main/BK/GBK004100');
      return;
    }
    __autoSearchedBukken = num;
    // フォーム描画待ち
    const waitFor = (fn, timeout = 15000) => new Promise(res => {
      const t0 = Date.now();
      const tick = () => {
        const r = fn();
        if (r) return res(r);
    ...
    // 物件種別 賃貸マンション/賃貸アパート/賃貸テラス・タウンハウス/賃貸戸建/賃貸その他 をすべて選ぶのは過剰
    // 物件番号フィールドを探して入力
    const input = await waitFor(() => {
      const inputs = [...document.querySelectorAll('input[type="text"], input:not([type])')];
      for (const el of inputs) {
        const parent = el.closest('.p-label, .form-group, div');
        const ctx = parent?.parentElement?.textContent || parent?.textContent || '';
        if (ctx.includes('物件番号')) return el;
      }
      return null;
    });
    if (!input) return;
    __autoSearchedBukken = num;
    // 物件種別を「賃貸」全般に設定（賃貸マンション 03 を選択）
    const sels = document.querySelectorAll('select');
    for (const sel of sels) {
      const opt = [...sel.options].find(o => o.value === '03' && /賃貸マンション/.test(o.text));
      if (opt) {
        sel.value = '03';
    ...
    // 物件番号を入力（Vueのv-modelに認識させるためnative setter経由）
    input.focus();
    try {
      const nativeSetter = Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype, 'value').set;
      nativeSetter.call(input, num);
    } catch(e) { input.value = num; }
    ...
    setTimeout(() => {
      ...
      if (btn) btn.click();
      // 完了したのでstorageクリア
      try { chrome.storage.local.remove(BUKKEN_KEY); } catch(e) {}
    }, 400);
  }
  window.addEventListener('hashchange', autoSearchByBukken);
  // 初回起動（DOM準備待ち）
  setTimeout(autoSearchByBukken, 1500);
  // SPA遷移後にも反応するためポーリング
  setInterval(autoSearchByBukken, 2500);
