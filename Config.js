   ...
   * LINE Channel Access Token と各種シートIDを管理する。
    * デプロイ前に自分の値に書き換えること。
     */
     ...
     // 空室確認モードに専用の有効期限は持たせない。
     // 以前は30分で切っていたが、お客様が数時間後に物件名を送ってきたときに
     // 何も返らず、壊れているように見えていた（2026-09-08）。
     // 会話状態そのものが CONVERSATION_TIMEOUT_MS（24時間）で消えるので、
     // 放置されたモードが後日のメッセージを拾う心配はそちらで防げている。
     // 相槌（「おねがいします」等）は VACANCY_FILLER_TEXTS で別途はじく。

     // 空室確認の回答をお客様に送るまで置く時間。
     // Discordで回答した瞬間に届くと、確認せず答えたように見えてしまう。
     // 実際に届くのは、返信キューのトリガー（5分間隔）の都合でこれ＋0〜5分。
     const VACANCY_ANSWER_DELAY_MS = 5 * 60 * 1000; // 5分
