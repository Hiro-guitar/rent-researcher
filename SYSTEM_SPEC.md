...
| 90009 | 定期借家を含まない | `lease_type` に「定期」を含む場合アラート | API option_id なし（仮ID）、**逆チェック** |
...
- **定期借家を含まない (90009):** 逆チェック — `lease_type` に「定期」を含む場合にアラートを表示する（他の設備は「見つからない場合」にアラート）。
