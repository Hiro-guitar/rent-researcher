...
| **REINS Chrome 拡張** (`chrome_extension/`) | Chrome Manifest V3 + Vue $data 操作 | REINS 半自動検索・物件取得 |
...
cleaning_fee, preview_start_date, image_categories,
...
5. 各顧客について:
   ...
   b. 通知済み・承認待ちを除外
   c. 各物件の詳細ページをスクレイピング（画像・設備・ステータス・WEBバッジ等、最大200件・ページネーション対応）
...
- 取得情報:
  ...
  - 構造 / 総戸数 / 主要採光面 / 設備・詳細
  - 入居可能時期 / 契約情報 / 費用関連
  - **追加費用**: 24時間サポート費、権利金、敷金積み増し、保証金、水道料金形態、駐車場代、駐輪場代、バイク置き場代、その他月次費用、その他一時金
  - **追加条件**: 入居条件、退去日、フリーレント詳細、間取り詳細
...
│ ⑦ _filter_by_structure() ← 建物構造条件で除外（ES-Square/いえらぶ）│
└────────────────────────────────────────────────────────────────────┘
  ↓
...
顧客が建物構造を指定している場合、詳細ページの構造テキストと照合して除外する。itandi BB は API の `structure_type:in` で検索時にフィルターするため不要。ES-Square・いえらぶBB の後処理で使用。
...
| `FORCE_NOTIFY` | `1` で通知済みチェックをスキップ（キャッシュ済み詳細を再利用） |
| `SKIP_CACHE` | `1` で FORCE_NOTIFY 時もキャッシュをスキップして再取得 |
| `TEST_MODE` | `1` でテスト顧客のみ・各3物件制限 |
| `SERVICE` | 実行サービス指定（`itandi` / `essquare` / `ielove`、未指定=全て） |
| `DRY_RUN` | `1` で LINE 送信をログ出力に切り替え（GAS スクリプトプロパティでも設定可） |
...
- MUI Grid パーサー: `MuiGrid-container` div を直接走査し、xs-4 (ラベル) / xs-8 (値) セルペアを抽出。未発見フィールドのみキーワードフォールバック
- 70以上の詳細フィールドを抽出（追加費用・入居条件等を含む）

**物件名・部屋番号の分離（GraphQL パーサー）:**
- 物件名末尾の部屋番号パターン（数字、英数字混在、1〜3文字の英字等）を正規表現で検出し、`building_name` と `room_number` に分離
- 例: `"メインステージ吉祥寺 1001"` → building_name=`"メインステージ吉祥寺"`, room_number=`"1001"`
...
**画像カテゴリ取得:**
- bxSlider 構造の画像から `alt` 属性等でカテゴリ（外観・間取り・内装等）を抽出
- property.html のカルーセルでカテゴリラベルを画像下部中央に表示

**設備情報のカテゴリ分類:**
- 「基本設備」「キッチン」「バス・トイレ」等のテーブルヘッダーをカテゴリ名として保持
- property.html でカテゴリ別タグ表示

### 4c.5 いえらぶBB固有のフィルター

**検索URL パラメータで指定するもの:**
- 建物構造: `kozou/{code}` パスセグメント（RC=4, SRC=5, 鉄骨=3, 軽量鉄骨=8, 木造=1, ブロック=2）
- ハード設備: `kdc/{code}` パスセグメント（ペット相談可, 事務所利用可, 家具家電付き）
- 敷金なし: `skk/1`
- 礼金なし: `rkn/1`

**一覧ページの後処理フィルター:**
- 敷金なし・礼金なし条件: 一覧の敷金/礼金テキストから `"なし"` / `"0"` を判定
- 駅名+徒歩の複合フィルタ: 複数交通情報から条件駅名を探し、徒歩分数を確認
...
「条件登録」入力
    │
  LINE Profile API で表示名を自動取得（名前入力ステップは廃止）
    │
  ...
  STEP_CRITERIA_SELECT → LIFF ページで詳細条件選択
    ...
    │                       面積、築年数、構造、設備・こだわり)
    │
  LIFF 送信 → 直接シートに保存 → 登録内容サマリー表示 → 完了メッセージ
  （確認画面ステップを経由せず即保存）
```
...
「条件変更」入力
  ...
  条件が見つからない場合 → 「まだ条件が登録されていません」メッセージ
    │
  条件が見つかった場合:
    state に既存条件を全復元（name, reason, resident, move_in_date,
    areaMethod, selectedRoutes, selectedStations, selectedCities 等）
    state.isChangeFlow = true をセット
    │
  現在の登録条件サマリーを表示（formatConditionSummary() で整形）
    │
  STEP_CRITERIA_SELECT → LIFF ページで条件変更（Flex Message）
    │                      ※ 名前・理由・居住者・引越し時期の入力をスキップ
    │                      ※ 既存の選択値がプリセットされた状態で表示
    │                      ※ ボタン: 「条件を変更する」「入居時期を変更」「キャンセル」
    │
    ├─ 「入居時期を変更」→ STEP_MOVE_IN_DATE → 変更後に即保存 → 完了メッセージ
    │
    └─ LIFF 送信 → 直接シートに保存 → 更新内容サマリー表示 → 完了メッセージ
       （確認画面ステップを経由せず即保存）
```

**関連関数:**
- `startChangeFlow(replyToken, userId)` (`ConversationFlow.js`): 条件変更フローの開始。既存条件を読み込み、サマリー表示後に CRITERIA_SELECT ステップに遷移。
- `formatConditionSummary(state)` (`ConversationFlow.js`): 入居時期・エリア・物件条件・こだわりを整形。条件変更前の現在値の表示に使用。
- `buildRegistrationSummary(state)` (`コード.js`): 登録/更新完了後の内容サマリーを整形。新規登録・条件変更の両方で使用。
...
| `get_criteria` | 検索条件取得 API（REINS 拡張用） | Chrome 拡張 |
| `get_seen_ids` | 通知済み物件ID取得 API（REINS 拡張用） | Chrome 拡張 |
| `track_view` | 物件閲覧トラッキング | property.html |
...
面積検索でヒットした物件のステータスが「募集中」以外
  ...
  LINE Push メッセージ送信
    │  Flex Message:
    │    本文: 「{物件名}について確認しましたが、ご案内が難しい状況でした。」
    │    +「似た条件の物件のご案内はご希望されますか？」
    │    フッター（横並び）:
    │      - 「はい」(緑) → message: 条件登録 → 条件登録フロー開始
    │      - 「いいえ」(灰) → message: 類似物件不要 → 「承知しました」返信
    │
  ステータスを sent に更新
```
...
**doPost のメッセージハンドラー (`コード.js`):**
- `「条件登録」` → `startSearchFlow()` を呼び出し（遅延返信 Flex の「はい」ボタン経由も同様）
- `「類似物件不要」` → 「承知いたしました。またお部屋探しの際はお気軽にお声がけください。」と返信（遅延返信 Flex の「いいえ」ボタン経由）

**古い Flex ボタンの誤操作防止:**
- LIFF ページ表示中に古い Flex メッセージの postback が送信された場合、`isCriteriaPageAllowed()` で無効と判定し、「LINEに戻って操作をやり直してください。」と返答する。
...
**表示内容:**
- 画像カルーセル（スワイプ対応、ドットインジケーター、カウンター表示、カテゴリラベル表示）
- 物件名・賃料・管理費
...
- **LINE送信前閲覧**: 承認前でも承認待ちシートのN列にお客さん用ページURLが生成され、管理者がプレビュー可能
...
**トリガー:**
- `repository_dispatch` ← cron-job.org からの HTTP ディスパッチ（30分おき）
- `workflow_dispatch` ← GitHub UI からの手動実行（`force_notify`, `test_mode`, `service` オプション）

**ランダムディレイ:**
- cron-job.org ディスパッチ時のみ、0〜10分のランダム遅延を挿入
- 手動実行時（`workflow_dispatch`）はスキップ

**SERVICE 選択（手動実行時）:**
- `all`（デフォルト）: itandi + ES-Square + いえらぶBB 全て実行
- `itandi`: itandi BB のみ
- `essquare`: ES-Square のみ
- `ielove`: いえらぶBB のみ
...
class Property:
    ...
    image_categories: list[str]        # 画像カテゴリ（外観・間取り・内装等、いえらぶBB用）
...
| 項目 | 値 |
|---|---|
| ベース URL | `https://system.reins.jp` |
| 検索フォーム | `/main/BK/GBK001310` |
| 詳細ページ | `/main/BK/GBK003200` |
| 技術基盤 | Vue 2 SPA (Nuxt.js) |
| アクセス方法 | Chrome 拡張経由（ブラウザログイン必須） |

### 9.6 LINE Messaging API
...
## 12. REINS Chrome 拡張 (`chrome_extension/`)

**REINS（レインズ）** の物件検索を半自動化する Chrome 拡張機能。ブラウザ内で REINS にログインした状態で動作し、検索条件シートの各顧客の条件に基づいて自動検索・詳細取得・GAS登録・Discord通知を行う。
...
├── background.js              # Service Worker（検索オーケストレーション）
├── content-search.js          # Content Script: 検索結果ページ解析
├── content-detail.js          # Content Script: 詳細ページ解析
├── popup.html / popup.js      # ポップアップ UI（ステータス・操作ボタン）
├── popup.css                  # ポップアップスタイル
├── options.html / options.js  # 設定ページ（GAS URL・APIキー・Webhook）
├── reinsCodeMap.json          # REINS路線表示名 → REINS内部コード（72路線）
└── lineNameMap.json           # 顧客条件の路線名 → REINS表示名
```

### 12.2 技術的アーキテクチャ

**Chrome 拡張 Manifest V3:**
- **権限**: `storage`, `alarms`, `tabs`, `notifications`, `scripting`
- **Host Permissions**: `https://system.reins.jp/*`, GAS URL
- **Content Scripts**: 検索結果ページ (`content-search.js`) + 詳細ページ (`content-detail.js`)
- **Service Worker**: `background.js`（メインロジック）

**REINS の技術的課題:**
REINS は **Vue 2 SPA (Nuxt.js)** で構築されており、通常の DOM 操作やテキスト入力では Vue のリアクティビティが正しく動作しない。

**解決策: Vue `$data` 直接代入 (MAIN world)**
```
chrome.scripting.executeScript({world: 'MAIN'})
  → document.querySelector('.p-textbox-input') から __vue__ を探索
  → 5段階 $parent 遡りでルートコンポーネント取得
  → $data に直接値を代入（bkknShbt1, ensnCd1, kkkuCnryuTo 等）
```

### 12.3 検索フロー

```
1. GAS API から顧客の検索条件を取得（get_criteria）
2. GAS API から顧客ごとの通知済み物件IDを取得（get_seen_ids）
3. 各顧客について:
   a. Vue Router で検索フォームページに遷移
      $nuxt.$router.push('/main/BK/GBK001310')
   b. Vue $data に検索条件を直接セット:
      - 物件種別: bkknShbt1 = '03'（賃貸マンション）
      - 沿線: ensnCd1〜3（最大3路線、4路線以上は分割検索）
      - 駅: ekCdFrom/ekCdTo + ekmiFrom/ekmiTo
      - 賃料上限: kkkuCnryuTo（万円単位）
      - 間取り: mdrTyp + mdrHysuFrom/To
      - 面積: snyuMnskFrom（建物使用部分面積）
      - 築年月: chkNngpFrom（年）+ chkNngpTukiFrom（月）
      - 駅徒歩: thNyurykc1 + thMHnKbn1
      - 所在階: shzikiFrom/To
   c. 検索ボタンをクリック
   d. 500件超ダイアログのOK自動クリック
   e. 検索結果を解析（最大200件、ページネーション対応）
   f. 各物件の詳細ページにアクセスしてデータ抽出
   g. フィルター適用（構造・駅名+徒歩・階数・敷金なし・礼金なし）
   h. GAS API で承認待ちシートに登録（add_reins_property）
   i. Discord に即時通知（物件ごと）
```

### 12.4 コンテンツスクリプト

**content-search.js（検索結果ページ）:**
- `.p-table-body-row` の32カラム構造を解析
- 物件番号・種目・面積・所在地・賃料・管理費・建物名・所在階・間取り・沿線駅・徒歩・築年月・商号・電話番号を抽出
- ページネーション操作（総ページ数取得・ページ遷移）
- 詳細ボタンクリック

**content-detail.js（詳細ページ）:**
- `.p-label-title` をラベルとして値を抽出する汎用パーサー
- 交通情報: 最大3件の沿線・駅名・徒歩を構造的に抽出
- 間取り: `madoriType` + `madoriRoomCount` → `"3LDK"` 形式
- 築年月: 西暦・和暦対応、年差計算で `"築10年"` に変換
- 画像: サムネイルクリック → `backgroundImage` URL 抽出
- 物件ID形式: `reins_{物件番号}`, `reins_{物件番号}_{部屋番号}`

### 12.5 GAS API エンドポイント（REINS 拡張用）

`コード.js` の `doGet` / `doPost` に追加されたエンドポイント:

| action | メソッド | 用途 |
|---|---|---|
| `get_criteria` | GET | 検索条件シートから全顧客の条件を取得（routes_with_stations 形式対応） |
| `get_seen_ids` | GET | 顧客ごとの通知済み+承認待ち物件IDセットを取得 |
| `add_reins_property` | POST | REINS物件を承認待ちシートに追加 |

**認証:** `REINS_API_KEY`（GAS スクリプトプロパティ）による Bearer トークン認証

### 12.6 Discord 通知

- 顧客ごとに Forum スレッドを作成（`🏠 {顧客名}`）
- スレッド最初に検索条件サマリーを投稿
- 各物件: 物件名、賃料（万円）、間取り、面積、築年数、住所、最寄り駅、敷金/礼金
- ソースバッジ: `[REINS]`
- 承認リンク: `GAS_URL?action=approve&customer=...&room_id=...`

### 12.7 路線コードマッピング

**2段階マッピング:**
1. `lineNameMap.json`: 顧客条件の路線名（例: `"JR 横須賀線"`）→ REINS 表示名（例: `"横須賀線"`）
2. `reinsCodeMap.json`: REINS 表示名 → REINS 内部コード（例: `"2179横須賀線"`）

- 72路線対応
- 検索不能な路線は `"検索不能"` でマーク（スキップ）
- 4路線以上の場合、3路線ずつに分割して複数回検索

### 12.8 設定項目

| 設定 | デフォルト | 説明 |
|---|---|---|
| GAS Web App URL | — | GAS デプロイメント URL（必須） |
| API Key | — | REINS_API_KEY（必須） |
| Discord Webhook URL | — | Forum チャンネル Webhook（任意） |
| 検索間隔 | 30分 | Chrome Alarms API で定期実行（10〜120分） |
| ページ遷移待ち | 2秒 | 詳細ページ間の待機時間 |
