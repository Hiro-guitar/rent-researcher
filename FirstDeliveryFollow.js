...
/**
 * カルーセルの**いちばん右**に置くバブル。希望を聞く一言と、条件変更のボタン。
 *
...
function buildFirstDeliveryFooterBubble(customerName) {
  ...
  // 今の条件。「この条件で探すとこうなる」が見えると、変更ボタンの意味がはっきりする。
  // ⚠️ 読めなかったら箱ごと出さない。空の表を見せるより何も無いほうがよい。
  try {
  ...
  bodyContents.push({ type: 'text', text: 'もう少しこういうお部屋がいい、などございましたら',
    size: 'sm', color: '#555555', wrap: true, margin: bodyContents.length ? 'lg' : 'none' });
  bodyContents.push({ type: 'text', text: 'お気軽にお申し付けください。',
    size: 'sm', color: '#555555', wrap: true });

  return {
    type: 'bubble',
    body: {
      type: 'box', layout: 'vertical', spacing: 'sm', paddingAll: 'xl', contents: bodyContents
    },
    footer: {
  ...
  for (var i = 0; i < rows.length; i++) {
    ...
    try {
      var r = resendPropertyNotifications(name, ids,
        buildFirstDeliveryResendText(), null, buildFirstDeliveryFooterBubble(name));
...
function testSendFirstDeliveryResend() {
  ...
  var r = resendPropertyNotifications(TO, ids,
    buildFirstDeliveryResendText(), null, buildFirstDeliveryFooterBubble(TO));
