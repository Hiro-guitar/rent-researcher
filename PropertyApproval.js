...
function makePreviewHtml(prop, customerName, roomId) {
  ...
  html += '<script>'
    ...
    + '// same-room (同建物+同タイプ=間取り+面積一致) のみ表示\n'
    ...
    + 'statusEl.textContent="✅ 同タイプ "+sameRoom.length+"件 (全体"+all.length+"件中 / 信頼度: "+conf+")";'
    + 'statusEl.style.color=sameRoom.length>0?"#27ae60":"#666";'
    ...
    + 'a.textContent=" 🔗 検索結果"+(urls.length>1?(si+1):"")+"を見る";'
    ...
    + 'sum.textContent="選択: 0 / "+list.length+"件";'
    ...
    + 'item.innerHTML="<input type=\\"checkbox\\" class=\\"cb\\"><img src=\\""+c.url+"\\" alt=\\"homes\\">";'
    ...
    + 'item.addEventListener("click",function(e){if(e.target!==cb){cb.checked=!cb.checked;toggle()}});'
