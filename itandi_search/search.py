...
def fetch_room_details(
    ...
    try:
        ...
        for (var li = 0; li < lines.length; li++) {
            ...
            if (/^(賃料|管理費|共益費|費用|契約|条件$|フリーレント|入居|現況|駐車場代|駐輪場代|バイク置き場代|水道|所在地|交通|物件概要|表示について|図面ダウンロード|物件資料|仲介|取引|敷金|礼金|保証会社|保証金|更新料|更新事務|解約|火災保険|備考|間取り|面積|専有|所在階|総戸数|方角|採光|入力なし|なし$|\\d+年|\\d+万|出稿|内見予約|WEB$|募集)/.test(ln)) { inFac = false; continue; }
