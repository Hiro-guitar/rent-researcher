...
// '追客中' は2026-09-09に「物件待ち / 自動のみ / やり取り中」の3つに分けた。
// どれも物件を探している人だが、こちらの手のかけ方が違う。
var KANBAN_STAGE_LIST = ['メールのみ', '未接続', '物件待ち', '自動のみ', 'やり取り中', '追客中（優先）', '申込', '成約', '終了'];
function _normalizeStageCell_(raw) {
  var v = String(raw == null ? '' : raw).trim();
  if (v === '問い合わせ') return '未接続';   // 旧名。列の意味は引き継ぐ
  if (v === '追客中') return '自動のみ';     // 旧名。分ける前の人はここに集める
 ...
 *   自動のみ   … それ以外（話せたことがある / LINEで繋がっている / メールでしか届かない）
 *
 * 「物件待ち」「やり取り中」には自動で入れない。条件が固まっているか、
 * やり取りが続いているかはデータから読めないので、担当者が動かして決める。
 *
...
function _fillDefaultStages_(customers) {
  for (var i = 0; i < customers.length; i++) {
    ...
    //   条件はあるがメールしか届かない人は「自動のみ」に置く。
    var everTalked = (c.daysSinceTalk !== null && c.daysSinceTalk !== undefined);
    c.stage = (c.hasPhone && !everTalked && !c.hasLine) ? '未接続' : '自動のみ';
  }
}
